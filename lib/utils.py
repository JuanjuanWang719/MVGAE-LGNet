...
def set_seed(seed: int, deterministic: bool = False) -> None:
    ...
    deterministic=True 会强制 cudnn 确定性算法，复现性更好，但在 GPU 上常慢 2～4 倍；
    默认 False：只固定 seed，保留 cudnn.benchmark 加速。
    """
    random.seed(seed)
    np.random.seed(seed)
...
def compute_len_input(
    num_of_weeks: int,
    num_of_days: int,
    num_of_hours: int,
    num_for_predict: int,
) -> int:
    """多周期输入总时间步：week + day + hour 各 num_for_predict 步。"""
    if num_of_weeks < 0 or num_of_days < 0 or num_of_hours < 0:
        raise ValueError("num_of_weeks/days/hours 不能为负")
    if num_of_weeks + num_of_days + num_of_hours <= 0:
        raise ValueError("至少启用 num_of_weeks / num_of_days / num_of_hours 之一")
    return (num_of_weeks + num_of_days + num_of_hours) * num_for_predict
...
def scaled_Laplacian(W):
    ...
    # scipy.sparse.linalg.eigs(..., which='LR') 在新版 SciPy 中已移除；此处用稠密特征值即可（N≈数百）
...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, seed=None):
    '''
    这个是为PEMS的数据准备的函数
    ...
    train_x = file_data['train_x']  # (B, N, F, T)，N/T 由数据集决定
    print('input time steps (T):', train_x.shape[-1])
    train_x = train_x[:, :, 0:1, :]
    train_target = file_data['train_target']  # (B, N, T_out)
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, metric_method,_mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        for i in range(prediction_length):
            if metric_method == 'mask':
                h_mae = masked_mae_test(data_target_tensor[:, :, i], prediction[:, :, i], 0.0)
                h_rmse = masked_rmse_test(data_target_tensor[:, :, i], prediction[:, :, i], 0.0)
                h_mape = masked_mape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
            else:
                h_mae = mean_absolute_error(data_target_tensor[:, :, i], prediction[:, :, i])
                h_rmse = mean_squared_error(data_target_tensor[:, :, i], prediction[:, :, i]) ** 0.5
                h_mape = masked_mape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
            horizon_metrics.append(
                {"horizon": i + 1, "mae": float(h_mae), "rmse": float(h_rmse), "mape": float(h_mape)}
            )
